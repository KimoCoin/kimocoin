...
uint256 hashGenesisBlock("0x5b97068f90a1a6071116d9e115d0331e9fcb8b1e0a0ca196003dc990db79d3d8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KimoCoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1396023706; // Line: 2815
...
const string strMessageMagic = "KimoCoin Signed Message:\n";
...
{
    // KimoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// KimoCoin: increasing Nfactor gradually
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight >= 1 && nHeight <= 3) {
        ...
        nSubsidy = 0.002 * COIN;
    } else if(nHeight >= 501 && nHeight <= 1500) {
		nSubsidy = 100 * COIN;
	} else if(nHeight >= 1501 && nHeight <= 2999) {
        nSubsidy = 200 * COIN;
	} else if(nHeight >= 3000 && nHeight <= 5000) {
        nSubsidy = 500 * COIN;
	...
	} else if(nHeight >= 100001 && nHeight <= 110000) {
        nSubsidy = 100000 * COIN;
	} else if(nHeight >= 110001 && nHeight <= 120000) {
        nSubsidy = 90000 * COIN;
	} else if(nHeight >= 120001 && nHeight <= 130000) {
        nSubsidy = 80000 * COIN;
	} else if(nHeight >= 130001 && nHeight <= 140000) {
        nSubsidy = 70000 * COIN;
	} else if(nHeight >= 140001 && nHeight <= 150000) {
        nSubsidy = 60000 * COIN;
	} else if(nHeight >= 150001) {
		nSubsidy >>= (nHeight / 7200);
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // KimoCoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // KimoCoin: 1 minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KimoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // KimoCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5b97068f90a1a6071116d9e115d0331e9fcb8b1e0a0ca196003dc990db79d3d8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Each generation wants new symbols, new people, new names. - Jim Morrison";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 7000579;
        block.nTime = 1396223940;
        ...
        assert(block.hashMerkleRoot == uint256("0x0561a762368ae599150a291aa2a6c5c061bc9fa16140458c3777ad37f72dd36b"));
...
unsigned char pchMessageStart[4] = { 0x6b, 0x69, 0x6d, 0x6f }; // KimoCoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KimoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KimoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KimoCoinMiner(CWallet *pwallet)
{
    printf("KimoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kimocoin-miner");
    ...
    try { loop {
        ...
        printf("Running KimoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KimoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KimoCoinMiner, pwallet));
}
